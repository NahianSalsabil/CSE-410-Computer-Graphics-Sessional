...
public:
	...
	Color getColorAtPoint(Vector3D intPoint)
	{
		return this->color;
	}
	...
	void ComputeCoefficients(Vector3D intersecting_point, Ray lightray, Ray ray_normal, Ray ray, PointLight *pointlight, Color intersect_point_color, Color *color)
	{
		...
		color->red += pointlight->getColor().red * getColorAtPoint(intersecting_point).red * coefficient.diffuse * lambert_value;
		color->green += pointlight->getColor().green * getColorAtPoint(intersecting_point).green * coefficient.diffuse * lambert_value;
		color->blue += pointlight->getColor().blue * getColorAtPoint(intersecting_point).blue * coefficient.diffuse * lambert_value;
	...
	double IntersectWithLighting(Ray ray, Color *color, int level);
	~Object()
	{
	}
};
...
double Object::IntersectWithLighting(Ray ray, Color *color, int level)
{
	...
	ray.origin = ray.origin.Add(reference_point);
	Vector3D intersecting_point = ray.origin.Add(ray.direction.Multiply(tmin));

	Color intersectpoint_color = getColorAtPoint(intersecting_point);
	...
	Vector3D intersecting_point_normal = intersecting_point.Subtract(reference_point);
	...
	for (int i = 0; i < pointLights.size(); i++)
	{
		...
		for (int j = 0; j < objects.size(); j++)
		{
			...
			double t = objects[j]->IntersectWithLighting(lightray, tempcolor, 0);
	...
	{
		Color *tempcolor = new Color();
		t = objects[i]->IntersectWithLighting(reflected_ray, tempcolor, 0);
		// cout << "t: " << t << endl;
		if (t > 0 && t < tMinimum)
		{
			tMinimum = t;
	...
	if (nearest != 9999999)
	{
		...
		objects[nearest]->IntersectWithLighting(reflected_ray, reflected_color, level + 1);
...
public:
	...
	{
		Vector3D normal = Vector3D(point.x - reference_point.x, point.y - reference_point.y, point.z - reference_point.z);
		normal.normalize();
		return normal;
	}
	...
	{
		// Intersection tMin //
		ray.origin = ray.origin.Subtract(reference_point); // translate the ray origin to the center of the sphere
...
public:
	...
	Color getColorAtPoint(Vector3D intPoint)
	{
		...
		int row = (reference_point.x + intPoint.x) / tile_width;
		int col = (reference_point.y + intPoint.y) / tile_width;
